...
public class JPanelMain extends JPanel implements Log, ActionListener, HyperlinkListener, ChangeListener {
     ...
     *
     */
    private static final long serialVersionUID = -4314053888240625799L;
    ...
    public JPanelMain() {
        ...
        try {
            ...
            for (Thread element : t) {
                ...
                if ("AWT-EventQueue-0".equals(element.getName())) {
                    element.setName("GUI");
                }
            }
        } catch (Exception e) {
    ...
    public void shutdown() {
        ...
        if (mWnio != null && A.conn != null && A.conn.m_authed) {
            ACon.shutdown = true;
            jbButt[JPanelMain.B_CONN].setEnabled(false); // disable the button
            mBnio = false;
    ...
    private void init() {
        /*
         * +---------------------------+ | BUTTONS | NORTH/TOP
         * +---------------------------+ |RLS|OPT|JOB|ALT|LOG|INF|DEB| | | CENTER |
         * (TABBED PANE) | | | +---------------------------+ | PROGRESSBARS! | SOUTH
         * +---------------------------+
         */

        //////////////////////////////// BUTTONS/////////////////////////////////
        jbButt = new JButton[JPanelMain.S_IBAL];
        ...
        for (int i = 0; i < jbButt.length; i++) {
            jbButt[i] = new JButton(JPanelMain.getName(i));
            jbButt[i].addActionListener(this);
            jbButt[i].setToolTipText(JPanelMain.getToolTip(i));
        ...
        final JList<String> jlExt = new JList<>(A.fha.m_ext);
        jlExt.getInputMap().put(KeyStroke.getKeyStroke("DELETE"), "pressed");
        jlExt.getActionMap().put("pressed", new AbstractAction() {

            private static final long serialVersionUID = -8735062245558873815L;
    ...
    /*
     * public void updateJobTable(Job j){ //jlmJobs.updateRow(j); }
     */
    ...
    public void nioEnable(boolean b) {
        ...
        if (!killed()) {
            ...
            if (b) {
                ...
                jbButt[JPanelMain.B_CONN].setText(JPanelMain.S_NIOE);
                mBnio = false;
            }
            jbButt[JPanelMain.B_CONN].setEnabled(true);
        }
    }
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if (source == jbButt[JPanelMain.B_SELF]) {
            selectFiles();
        } else if (source == jbButt[JPanelMain.B_SELD]) {
            selectDirs();
        } else if (source == jbButt[JPanelMain.B_HASH]) {
            dioToggle();
            ...
            mWdiv.start();
        } else if (source == jbButt[JPanelMain.B_SAVE]) {
            opts(A.opt);
            A.opt.save();
        } else if (source == jbButt[JPanelMain.B_WIKI]) {
    ...
    public void dioToggle() {
        ...
        if (mBdio) {
            jbButt[JPanelMain.B_HASH].setText(JPanelMain.S_DIOD);
            dioStart();
            mTdio.start();
        } else {
            jbButt[JPanelMain.B_HASH].setText(JPanelMain.S_DIOE);
            mTdio.stop();
        }
    }
    ...
    /*
     * public void dioStop(){ mBdio = false; }
     */
    private void nioToggle() {
        ...
        if (mBnio) { // we want it to run
            ...
            if (!mBnio) { // failed
                jbButt[JPanelMain.B_CONN].setEnabled(true); // enable the button
        ...
        public void run() {
            ...
            for (File dir : dirs) {
                ...
                if (parent != null && parent.startsWith("\\\\")) {
                    msg("Windor network paths not supported: " + dir);
                } else {
                    nr += addFileRecursive(dir);
                }
            }
    ...
    private final static int B_WIKI = 0, B_SELF = 1, B_SELD = 2, B_HASH = 3, B_CONN = 4, B_SAVE = 5, B_EXPO = 6,
            B_IMPO = 7, S_IBAL = 8;
    ...
    private static String getName(int code) {
        ...
        case B_SELF -> "Files...";
        case B_SELD -> "Folders...";
        case B_HASH -> JPanelMain.S_DIOE;
        case B_CONN -> JPanelMain.S_NIOE;
        case B_SAVE -> "Save opt";
        case B_WIKI -> "Help!";
        case B_EXPO -> "Export";
        case B_IMPO -> "Import";
        default -> "No text!";
    ...
    private static String getToolTip(int code) {
        return switch (code) {
        case B_SELF -> "Add files you want to hash";
        case B_SELD -> "Add folders with files you want to hash";
        case B_HASH -> "Start/stop the disk operations thread. (Hashing and moving)";
        case B_CONN -> "Log on / log off the AniDB UDP Service";
        case B_SAVE -> "Save the options to disk";
        case B_WIKI -> "Check out the documentation @ AniDB WIKI";
        case B_EXPO -> "Export loaded data";
        case B_IMPO -> "Import exported data";
        default -> "No help!";
        };
    }
}
