...
public class JobMan {
    public static void setPath(Job job, String path, boolean parent) {
        ...
        synchronized (job) {
            ...
            if (parent) {
                name.append(orig.getParentFile().getName()).append(File.separatorChar);
            }
            name.append(orig.getName());
            File file = new File(folder + File.separator + name.toString());
    ...
    public static void updateStatus(Job job, int status, boolean chck) {
        ...
        synchronized (job) {
            ...
            case Job.HASHED:
                ...
                if (job.mBf) {
                    ...
                    if (A.db._ok() && !A.db.update(0, job, DB.I_J)) { // add job to db, if it exist...
                        ...
                        if (j >= 0) { // if job exist in db... (should be true, always)
                            ...
                            if (JobMan.updatePath(job)) { // apply rules

                                if (job.m_fn != null) {
                                    j = Job.MOVEWAIT; // we want to move to a new location, is it the same as the orig?
                                    // if(old!=null&&!job.nfile.equals(old)&&job.getFile().exists()) //if not then
                                    // delete orig
                                    // A.deleteFileAndFolder(old, "Dupe of "+job.getFile());
                                } else {
                                    j = Job.FINISHED;
                                    // if(old!=null&&!job.getFile().equals(old)&&job.getFile().exists()) //delete
                                    // orig file if it exist and does not equal the new
                                    // A.deleteFileAndFolder(old, "Dupe of "+job.getFile());
                                }
                            } else { // apply rules
                                j = Job.FAILED;
                ...
                break;
            case Job.IDENTIFIED:
                if (job.m_fa == null || job.m_fa.fid == 0 || !JobMan.updatePath(job)) {
                    j = Job.FAILED;
...
//THE END
    ...
    private static void moveSub(File a, File b) {
        JobMan.moveSub(a, b, "srt");
        JobMan.moveSub(a, b, "ass");
        JobMan.moveSub(a, b, "idx");
        JobMan.moveSub(a, b, "sub");
        JobMan.moveSub(a, b, "pdf");
        JobMan.moveSub(a, b, "ssa");
    }

    private static void moveSub(File a, File b, String ext) {
        File c = new File(JobMan.changeExt(a, ext));

        if (c.exists()) {
            c.renameTo(new File(JobMan.changeExt(b, ext)));
        }
    }
