...
public class Options {
    ...
    private File m_file;
    public int[] mIa = new int[Options.I_LEN];
    public String[] mSa = new String[Options.S_LEN];
    public boolean[] mBa = new boolean[Options.B_LEN];
    ...
    public String enc() {
        return Options.S_VER + Options.S_SEP2 + encB() + Options.S_SEP2 + encI() + Options.S_SEP2 + encS();
    }

    public boolean dec(String str) {
        StringTokenizer st = new StringTokenizer(str, Options.S_SEP2);

        if (st.countTokens() > 0 && Options.S_VER.equals(st.nextToken())) {
            decB(st.nextToken());
            decI(st.nextToken());
            decS(st);
            return true;
    ...
    private String encS() {
        ...
        for (int i = 0; i < Options.S_LEN; i++) {
            s.append((mSa[i] == null || mSa[i].length() == 0) ? "null" : mSa[i]).append(Options.S_SEP2);
        }
        return s.toString();
    }
    ...
    private void decS(StringTokenizer st) {
        ...
        for (int i = 0; i < Options.S_LEN; i++) {
            ...
            mSa[i] = (tmp == null || "null".equals(tmp)) ? "" : tmp;
        }
    }
    ...
    public static final int B_UNUSED0 = 0, //
            ...
            I_RPORT = 0, I_LPORT = 1, I_STATE = 2, // file state on mylist add
            I_TIMEO = 3, // timeout
