...
public class JobList {
    ...
    /*
     * public void dumpHashSet(){ try { Iterator it = m_al.iterator(); FileWriter
     * fw;
     *
    ...
    public synchronized void filter(int status, int state, boolean unk) {
        ...
        ArrayList<Job> al = new ArrayList<>(m_al.size());
        Job j;
    ...
    public Job getJobDio() {
        return (Job) m_lh[JobList.DIO].getFirst();
    }

    public Job getJobNio() {
        return (Job) m_lh[JobList.NIO].getFirst();
    }

    public boolean workForDio() {
        return m_lh[JobList.DIO].size() > 0;
    }

    public boolean workForNio() {
        return m_lh[JobList.NIO].size() > 0;
    }
