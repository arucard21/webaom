...
public class AConE extends ACon {
    ...
    public String[] retrieveFileData(long len, String ed2k, String name) throws AConEx {
        return retrieveFileData(send("FILE", "size=" + len + "&ed2k=" + ed2k + AConE.facode, true),
                "ed2k://|file|" + name + "|" + len + "|" + ed2k + "|");
    }

    public String[] retrieveFileData(int fid, String name) throws AConEx {
        return retrieveFileData(send("FILE", "fid=" + fid + AConE.facode, true), name);
    }
    ...
    public int addFileToMylist(Job j, Mylist m) throws AConEx {
        ...
        sb.append(m.vie);
        sb.append("&source=");
        sb.append(AConE.validate(m.sou));
        sb.append("&storage=");
        sb.append(AConE.validate(m.sto));
        sb.append("&other=");
        sb.append(AConE.validate(m.oth));
        sb.append("&edit=0");
    ...
    /*
     * public Anime getAnime(int aid) throws AConEx{ AConR r = send("ANIME",
     * "aid="+aid, true); if(r!=null && r.code==AConR.ANIME) return
     * Parser.parseAnime(U.split(r.data,'|')); return null; } public Group
     * getGroup(int gid) throws AConEx{ AConR r = send("GROUP", "gid="+gid, true);
     * if(r!=null && r.code==AConR.GROUP) return
     * Parser.parseGroup(U.split(r.data,'|')); return null; } public Ep
     * getEpisode(int eid) throws AConEx{ AConR r = send("EPISODE", "eid="+eid,
     * true); if(r!=null && r.code==AConR.EPISODE) return
     * Parser.parseEpisode(U.split(r.data,'|')); return null; }
     */
}
