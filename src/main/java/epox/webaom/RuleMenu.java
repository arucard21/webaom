...
public class RuleMenu extends DefaultHandler {
    ...
    private class MyListener/* extends MouseAdapter */ implements ActionListener {
         ...
         * m_pb.setString(m_item.getToolTipText()); } public void mouseExited(MouseEvent
         * e) { super.mouseExited(e); m_pb.setString(""); }
         */
    }
    /*
     * protected final JProgressBar m_pb=new JProgressBar(); protected JTextArea
     * m_tf = new JTextArea(); private static Font MYF = new Font("Tahoma",
     * Font.PLAIN, 11); public static void main (String [] args) { final RuleMenu
     * handler = new RuleMenu(); SAXParserFactory factory =
     * SAXParserFactory.newInstance(); try { SAXParser saxParser =
     * factory.newSAXParser(); saxParser.parse( new File("C:\\rule-helper.xml"),
     * handler );
     *
     * final JPopupMenu x = handler.getMenu(); x.setFont(MYF);
     *
     * JFrame f = new JFrame("test");
     * f.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE); handler.m_tf.setFont(new
     * Font("Tahoma", Font.PLAIN, 18)); handler.m_tf.setMargin(new Insets(4,4,4,4));
     * handler.m_tf.addMouseListener(new MouseAdapter(){
     *
     * @Override public void mouseClicked(MouseEvent e) { super.mouseClicked(e);
     * if(e.getButton()==MouseEvent.BUTTON3) x.show(handler.m_tf,
     * e.getX(),e.getY()); } }); f.getContentPane().setLayout(new BorderLayout());
     * f.getContentPane().add(handler.m_tf, BorderLayout.CENTER);
     * f.getContentPane().add(handler.m_pb, BorderLayout.SOUTH);
     * handler.m_pb.setStringPainted(true); f.setBounds(100,200, 640, 480);
     * f.setVisible(true);
     *
     * handler.m_tf.
