 ...
 * package only because it came without a package. I was not the the orig author
 * of the code, although I did optimize it (substantially) and fix some bugs.
 * <p>
...
public class MD5 extends jonelo.jacksum.algorithm.AbstractChecksum {
     ...
     */
    /*
     * public MD5 (Object ob) { this(); Update(ob.toString()); }
     */
    ...
    private void Transform(MD5State mstate, byte buffer[], int shift, int[] decode_buf) {
        int a = mstate.state[0], b = mstate.state[1], c = mstate.state[2], d = mstate.state[3], x[] = decode_buf;
     ...
     * @param len    Use at maximum `length' bytes (absolute maximum is
     *               buffer.length)
     */
    public void Update(MD5State stat, byte buffer[], int offset, int len) {
        ...
        if (len >= partlen) {
             ...
             *
             * if (partlen == 64) { partlen = 0; } else { for (i = 0; i < partlen; i++)
             * stat.buffer[i + index] = buffer[i + offset]; Transform_native(stat.state,
             * stat.buffer, 0, 64); } Transform_native(stat.state, buffer, partlen + offset,
             * length - partlen); i = partlen + ((length - partlen) / 64) * 64; } else
             */ {
     ...
     * @param charset_name The character set to use to convert s to a byte array, or
     *                     null if the "ISO8859_1" character set is desired.
     * @exception java.io.UnsupportedEncodingException If the named charset is not
     *                                                 supported.
     */
    ...
    public synchronized byte[] Final() {
        ...
        if (finals == null) {
            ...
            Update(fin, MD5.padding, 0, padlen);
            Update(fin, bits, 0, 8);
    ...
    /*
     * public static synchronized final void initNativeLibrary(boolean
     * disallow_lib_loading) { if (disallow_lib_loading) { native_lib_init_pending =
     * false; } else { _initNativeLibrary(); } }
     *
     * private static synchronized final void _initNativeLibrary() { if
     * (!native_lib_init_pending) return; native_lib_loaded = _loadNativeLibrary();
     * System.out.println(native_lib_loaded); native_lib_init_pending = false; }
     *
     * private static synchronized final boolean _loadNativeLibrary() { try {
     *
     * // don't try to load if the right property is set
     *
     * String prop = System.getProperty("com.twmacinta.util.MD5.NO_NATIVE_LIB"); if
     * (prop != null) { prop = prop.trim(); if (prop.equalsIgnoreCase("true") ||
     * prop.equals("1")) return false; }
     *
     * // the library to load can be specified as a property
     *
     * File f; prop = System.getProperty("com.twmacinta.util.MD5.NATIVE_LIB_FILE");
     * if (prop != null) { f = new File(prop); if (f.canRead()) {
     * System.load(f.getAbsolutePath()); return true; } }
     *
     * // determine the operating system and architecture
     *
     * String os_name = System.getProperty("os.name"); String os_arch =
     * System.getProperty("os.arch"); if (os_name == null || os_arch == null) return
     * false; os_name = os_name.toLowerCase(); os_arch = os_arch.toLowerCase();
     *
     * // define settings which are OS arch architecture independent
     *
     * File arch_lib_path = null; String arch_libfile_suffix = null;
     *
     * // fill in settings for Linux on x86
     *
     * if (os_name.equals("linux") && (os_arch.equals("x86") ||
     * os_arch.equals("i386") || os_arch.equals("i486") || os_arch.equals("i586") ||
     * os_arch.equals("i686"))) { arch_lib_path = new File(new File(new File("lib"),
     * "arch"), "linux_x86"); arch_libfile_suffix = ".so";
     *
     * // fill in settings for Windows on x86
     *
     * } else if (os_name.startsWith("windows ") && (os_arch.equals("x86") ||
     * os_arch.equals("i386") || os_arch.equals("i486") || os_arch.equals("i586") ||
     * os_arch.equals("i686"))) { arch_lib_path = new File(new File(new File("lib"),
     * "arch"), "win32_x86"); arch_libfile_suffix = ".dll";
     *
     * // default to .so files with no architecture specific subdirectory
     *
     * } else { arch_libfile_suffix = ".so"; }
     *
     ...
     *
     * if (arch_lib_path != null) { f = new File(arch_lib_path, fname); if
     * (f.canRead()) { System.load(f.getAbsolutePath()); return true; } }
     *
     * // try the "lib" subdirectory
     *
     * f = new File(new File("lib"), fname); if (f.canRead()) {
     * System.load(f.getAbsolutePath()); return true; }
     *
     * // try the working directory
     *
     * f = new File(fname); if (f.canRead()) { System.load(f.getAbsolutePath());
     * return true; }
     *
